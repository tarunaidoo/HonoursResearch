...
class LLMInterface:
    ...
    def __init__(self, api_key, model_name="gpt-3.5-turbo"):
        # NEW: Check if the calling class is explicitly passing a mock key
        if api_key == "mock":
            self.client = None
            self.model_name = model_name
            return
    ...
    def construct_prompt(self, maze: Maze, prompt_type="zero_shot", maze_format="ascii", examples=None):
        ...
        base_instruction = (
            "TASK: Find the complete, step-by-step path from START to GOAL. "
            "The maze grid uses **0 for free space** and **1 for walls/obstacles**. "
            "CRITICAL RULES: "
            "1. MOVEMENT: Only move to **adjacent cells** (Up, Down, Left, Right). **NO DIAGONALS**. "
            "2. CONSTRAINTS: **NEVER** step on a wall (1) or move outside the maze boundaries. "
            "3. OUTPUT FORMAT: The final output **MUST BE ONLY** the Python list of (row, column) tuples, "
            "starting at START and ending at GOAL. Example: [(0, 0), (0, 1), (1, 1), ...]"
        )
        ...
        if prompt_type == "zero_shot":
            ...
            # 2. ADDED/REFINED DIRECTIVE FOR THOUGHT PROCESS (Focus on wall-checking)
            prompt = (
                f"{cot_instruction}\n\n"
                ...
                f"generate a step-by-step plan. For EACH STEP, explicitly state the current cell, the chosen next adjacent cell, and **CONFIRM that the chosen cell IS NOT a wall/obstacle ('#', '1')**. "
                f"Finally, provide ONLY the complete Python list for the path.\n\n"
                f"Maze:\n{maze_representation}\n\n"
                f"Thought Process (Dimensions, Start, Goal, **Wall-Checked Step-by-Step Plan**, Final Path List):"
            )
        else:
    ...
    def _simplify_path(self, path):
        """Removes simple, immediate back-and-forth loops from a path, e.g., (A, B, A) -> (A)."""
        ...
        for i in range(1, len(path)):
            ...
            # Check if the current coordinate is the same as the one before the last one
            if len(simplified) >= 2 and current_coord == simplified[-2]:
                simplified.pop() # Remove the previous coordinate (B)
            else:
    ...
    def parse_llm_output(self, output_text):
        """
        Parses the LLM's output string into a list of (row, col) tuples and simplifies it.
        """
        try:
    ...
    def get_llm_plan(self, maze: Maze, model_name=None, prompt_type="zero_shot", maze_format="ascii", examples=None, max_tokens=2048, temperature=0.7):
        ...
        try:
            ...
            # Print the exact error that caused the API call to fail
            error_message = f"[LLM API ERROR] Model: {final_model}, Format: {maze_format}, Prompt: {prompt_type} | CRITICAL EXCEPTION: {e}"
            print(error_message) 
            # --- END OF ADDED DEBUGGING CODE ---
            
            # Return failure values, which lead to token_usage=0.0 in main.py
...
class MockLLMInterface:
    def __init__(self, api_key=None, model_name=None):
        ...
        # NOTE: We pass a mock key because LLMInterface.__init__ now raises a ValueError if none is found.
